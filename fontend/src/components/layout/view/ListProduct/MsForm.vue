    ...
    <div class="form">
      ...
      <div class="form-center">
        <div class="row-flex">
          <div class="form-small-div col-flex">
            ...
            <input
              ...
              maxlength="20"
            />
            <label v-show="checkNull('PropertyCode')" class="text-error"
          ...
          <div class="form-big-div col-flex">
            ...
            />
            <label v-show="checkNull('PropertyName')" class="text-error"
              >Cần phải nhập thông tin</label
            >
          </div>
        </div>
        ...
        <div class="row-flex">
          <div class="form-small-div col-flex">
            <label class="Form-Label"
              >Số lượng <span style="color: red">*</span></label
            >
            <div
              style="
                align-items: center;
            ...
            <label v-show="checkNull('Quantity')" class="text-error"
          ...
          <div class="form-small-div col-flex">
            <label class="Form-Label"
              >Nguyên giá <span style="color: red">*</span></label
            >
            <input
              type="text"
              class="form-input input-number"
            ...
            />
            <label v-show="checkNull('MarkedPrice')" class="text-error"
              >Cần phải nhập thông tin</label
            >
          </div>
          <div class="form-small-div col-flex">
            <label class="Form-Label"
              >Số năm sử dụng <span style="color: red">*</span></label
            >
            <input
              type="text"
              class="form-input input-number"
              ...
              value="0"
            />
            <label v-show="checkNull('UsedYear')" class="text-error"
              >Cần phải nhập thông tin
            </label>
          </div>
        </div>
        <div class="row-flex">
          <div class="form-small-div col-flex">
            <label class="Form-Label"
              >Tỉ lệ hao mòn (%) <span style="color: red">*</span></label
            >
            <div
              style="
                align-items: center;
            ...
            >
              ...
              <div class="input-btn-updown">
                <div
                  class="icon icon-mini-up"
                  @click="upAttritionRate()"
                  style="cursor: pointer"
                ></div>
                <div
                  class="icon icon-mini-down"
                  @click="downAttritionRate()"
                  style="cursor: pointer"
                ></div>
              </div>
            </div>
            <label v-show="checkNull('AttritionRate')" class="text-error"
              >Cần phải nhập thông tin</label
            >
          </div>
          <div class="form-small-div col-flex">
            <label class="Form-Label"
              >Giá trị hao mòn năm <span style="color: red">*</span></label
            >
            <input
              type="text"
              class="form-input input-number"
              ...
              :disabled="item.IsActive == 1"
            />
            <label v-show="checkAttritionValuePerYear()" class="text-error"
              >Cần phải nhập thông tin</label
            >
          </div>
          <div class="form-small-div col-flex">
        ...
        <div class="row-flex">
          <div class="form-small-div col-flex">
            ...
            <label v-show="checkNull('PurchasingDate')" class="text-error"
              >Cần phải nhập thông tin</label
            >
          </div>
          <div class="form-small-div col-flex">
            ...
            <label v-show="checkNull('StartUsingDate')" class="text-error"
              >Cần phải nhập thông tin</label
            >
          </div>
...
export default {
  ...
  methods: {
    ...
    calculateAttritionValue() {
      ...
      if (numberUsedYear > 0) {
        var temp =
          this.item.MarkedPrice * numberUsedYear * this.item.AttritionRate;
    ...
    inputMarkedPrice() {
      ...
      this.AttritionValuePerYear.set(
        this.item.MarkedPrice * this.item.AttritionRate
      );
    ...
    inputAttritionValuePerYear() {
      ...
      if (valueNumber > this.item.MarkedPrice) {
        this.emitter.emit(
          "openPopupNotice",
          Resource.PopupNotice.ErrorAttritionValue
        );
    ...
    checkAttritionRateAndUsedYear() {
      if (this.item.AttritionRate * (this.item.UsedYear - 1) >= 1) {
        this.emitter.emit(
          "openPopupNotice",
          Resource.PopupNotice.ErrorAttritionRate
        );
    ...
    checkStartUsingDate() {
      ...
      if (
        ...
        this.emitter.emit(
          "openPopupNotice",
          Resource.PopupNotice.ErrorStartUsingDate
        );
        this.check = true;
      }
    },
    ...
    checkNull(key) {
      if (
        this.check == true &&
        (this.item[key] == null || this.item[key] == 0 || this.item[key] == "")
      ) {
        return true;
      }
      return false;
    ...
    callAutoNumeric() {
      ...
      this.AttritionValuePerYear = new AutoNumeric(
        this.$refs.inputAttritionValuePerYear,
        {
          decimalCharacter: Resource.DecimalCharacter,
          digitGroupSeparator: Resource.DigitGroupSeparator,
          decimalPlaces: "2",
    ...
    async getNewCode() {
      await apiGetNewCode()
        ...
        .catch(() => {
          this.emitter.emit(
            "openPopupNotice",
            Resource.PopupNotice.ErrorGetNewCode
          );
        });
    },
    /**
    ...
    async getPropertyById(id) {
      await apiGetPropertyByID(id)
        ...
        .catch(() => {
          this.emitter.emit(
            "openPopupNotice",
            Resource.ErrorMessage.ErrorMessageGetProperty
          );
        });
    },
    /**
      ...
      await apiAddNewProperty(this.item)
        ...
        .catch((e) => {
          if (e.response.data == Resource.ErrorCode.DuplicateCode) {
            this.emitter.emit(
              "openPopupNotice",
              Resource.PopupNotice.ErrorDuplicate
            );
          } else {
            this.emitter.emit(
    ...
    async updateProperty() {
      await apiUpdateProperty(this.item.PropertyID, this.item)
        .then(() => {
          ...
          this.emitter.emit(
            "openToastMessage",
            Resource.SuccessMessage.SuccessMessageFix
          );
          this.emitter.emit("loadNewTable");
        })
        .catch((e) => {
          if (e.response.data == Resource.ErrorCode.DuplicateCode) {
            ...
            this.emitter.emit(
              "openToastMessageError",
              Resource.ErrorMessage.ErrorMessageFix
            );
          }
        });
    },
  ...
  async created() {
     ...
     * NTD 19/10/2022
     */
    await apiGetBudget().then((response) => {
       ...
       * NTD 5/9/2022
       */
      await apiGetPropertyType().then((response) => {
