    ...
    >
      <thead>
        <tr>
          ...
          <th class="col-code">Mã nhãn dán</th>
          <th class="col-name">Tên nhãn dán</th>
          <th class="col-type">Loại nhãn dán</th>
          <th class="col-description">Diễn giải</th>
      ...
      <tbody ref="tbody">
        ...
        <div
          v-if="datalist.length == 0 && isLoader == false"
          style="
            position: relative;
            text-align: center;
            align-items: center;
        ...
        <tr>
          <td style="width: auto; font-size: 11px">
            <div style="margin-left: 16px">
              Tổng số: <b style="font-weight: 700">{{ totalItem }}</b> bản ghi
            </div>

            <div
              class="select-number-footer"
              @click="showSelect = true"
              v-click-outside="closeSelect"
            >
              <div class="option-box" v-show="showSelect">
                <div
                  class="option-item"
                  @click.stop="changeNumberItemInTable(10)"
                >
                  10
                </div>
                <div
                  class="option-item"
                  @click.stop="changeNumberItemInTable(20)"
                >
                  20
                </div>
                <div
                  class="option-item"
                  @click.stop="changeNumberItemInTable(50)"
                >
                  50
                </div>
              </div>
...
<script>
import CategoryRow from "@/components/base/CategoryRow.vue";
import CategoryTool from "./CategoryTool.vue";
import VPagination from "@hennge/vue3-pagination";
import "@hennge/vue3-pagination/dist/vue3-pagination.css";
import Resource from "@/resource/MsResource";
import { apiDeleteByID, apiDeleteMultiple } from "@/api/modules";
...
import {
  ...
  closeSelect,
  checkItemInList,
} from "@/method/methodTable";
import { apiGetPagingCategory } from "@/api/categoryApi";
...
export default {
  ...
  components: { CategoryRow, VPagination, CategoryTool },
  methods: {
    /**
    ...
    async getNewData() {
      ...
      await apiGetPagingCategory(
        filter,
        this.numberItemInTable,
        this.pageNumber
      )
        .then((response) => {
          debugger;
    ...
    async deleteItemById(id) {
      await apiDeleteByID(id)
        .then(() => {
          this.emitter.emit(
            "openToastMessage",
            Resource.SuccessMessage.SuccessMessageDelete
          );
          this.getNewData();
        })
        .catch(() => {
          this.emitter.emit(
            ...
            Resource.ErrorMessage.ErrorMessageDelete
          );
        });
    },
    /***
