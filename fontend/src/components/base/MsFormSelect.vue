<template>
    <div class="row-flex">
        <div class="form-small-div col-flex">
            <label class="Form-Label">{{label1}} <span style="color:red">*</span></label>
            <div class="combobox"  
                @click="changeIsOpen()" 
            ...
            >
                <div class="box-input" :class="{'box-input-select': isOpen, 'error': check }" style="padding-left: 14px;">
                    <input type="text" class="input-box form-placeholder" 
                        
                        :value="inputCode" 
                        :placeholder="placeholder"
                        @input="isOpen = true ;$emit('update:inputCode', $event.target.value)"
                        ref ="input"
                        style="cursor:pointer; "
                    >
                    <div class="icon icon-mini-down" style="position: absolute; right:12px; top:15px"></div>
                </div>
                <div class="box-item " :class="{'box-item-display': isOpen}" ref="boxItem" :style="styleBoxItem">
                    ...
                    <div class="item-box" 
                        ...
                        :class="{'highlight': index == tmpIndex, 'selected':item.name === inputCode}" 
                    > 
                        <div class="item-code">
                            {{item.code}}
                        </div>
            ...
            <label v-if="check" style=" color:red; font-size: 11px">Cần phải nhập thông tin </label>
        </div>
        
        <div class="form-big-div col-flex">
            <label class="Form-Label">{{label2}}</label>
            <input type="text" class="form-input" style="background-color: rgb(245,245,245);" readonly :value="inputName" disabled>
        </div>
    </div>
</template>
<script>
    ...
    methods: {
        ...
        checkInput() {
            ...
            if(this.inputCode == null || this.inputCode =='' ) {
                ...
                this.items.forEach(element => {
                    if(element.name.toLowerCase().includes(this.inputCode.toLowerCase()) || element.code.toLowerCase().includes(this.inputCode.toLowerCase())) {
